...
import { does_cache_file_exist, fetch_json_and_write_to_cache, get_nist_compounds_data, write_to_cache_json } from "../filesystem_integration"
...
// await type_helper(nist_data) // made this type thanks to that.
export type CommonChemistryCompound = {
    uri: string
    rn: string
    name: string
    ...
    experimentalProperties: {[key: string]: any}
    propertyCitations: {[key: string]: any}
    synonyms: {[key: string]: any}
    replacedRns: {[key: string]: any}
    hasMolfile: boolean
} | {
    no_cas_number: true
} | {
